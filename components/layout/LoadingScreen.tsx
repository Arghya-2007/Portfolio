...
// Register CustomEase if not already done in gsap.config, but we can also use power3
// We will use power4 or custom eases directly available in GSAP.
...
export default function LoadingScreen() {
  ...
  const loaderMainRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const updateProgress = (now: number) => {
      ...
      if (progressRef.current >= 100 && assetsReadyRef.current) {
        ...
        setTimeout(() => {
          // Play premium GSAP exit transition
          ...
          // Custom ease mimicking the [0.77, 0, 0.175, 1] cubic bezier from framer motion
          const exitEase = CustomEase.create("exitEase", "0.77, 0, 0.175, 1")

          // Fade out loader UI elements
          tl.to(loaderMainRef.current, {
            opacity: 0,
            duration: 0.8,
            ease: "power2.inOut"
          }, 0)
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4 -mt-4">
          ...
          <h1
            ref={titleRef}
            className="font-display font-bold text-2xl sm:text-3xl tracking-tight text-grad-warm mb-2 opacity-0 translate-y-[10px]"
          >
            Arghya Pal
          </h1>
      ...
      <div className="fixed inset-0 z-[99998] pointer-events-none flex flex-col">
        {/* Top Panel */}
        <div
          ref={topCurtainRef}
