...
const DotGrid: React.FC<DotGridProps> = ({
    ...
    const draw = () => {
      ...
      for (let i = 0; i < dotsRef.current.length; i++) {
        ...
        if (dsq <= proxSq) {
          ...
          activeDots.push({ ox, oy, r, g, b });
        } else {
          ctx.moveTo(ox + halfSize, oy);
          ctx.arc(ox, oy, halfSize, 0, PI2);
        }
      }
      ctx.fill();
      ...
      for (let i = 0; i < activeDots.length; i++) {
        ...
        ctx.beginPath();
        ctx.arc(dot.ox, dot.oy, halfSize, 0, PI2);
        ctx.fill();
      }
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        const dx = dot.cx - cx;
        const cyDiff = dot.cy - cy;
        const distSq = dx * dx + cyDiff * cyDiff;
        ...
        if (distSq < shockSq && !dot._inertiaApplied) {
          ...
          const pushY = cyDiff * shockStrength * falloff;
          gsap.to(dot, {
            inertia: { xOffset: pushX, yOffset: pushY, resistance },
            onComplete: () => {
              gsap.to(dot, {
